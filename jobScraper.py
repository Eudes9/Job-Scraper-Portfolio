...
def saveToExcel(jobList, jobListNoDetails, jobListRejected, timePeriod):
    ...
    # Get the directory where jobScraper.py is located
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

    # Ensure a "data" folder exists inside that directory
    data_dir = os.path.join(BASE_DIR, "data")
    os.makedirs(data_dir, exist_ok=True)

    # Build the full path for the Excel file
    filename = os.path.join(data_dir, f'jobListings-{timePeriod}-{current_time}.xlsx')
